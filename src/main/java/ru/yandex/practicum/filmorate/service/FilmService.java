...
public class FilmService {
    ...
    public void addLike(int filmId, int userId) {
        List<Film> currentList = filmStorage.getFilms();
        //проверяем корректность id указанных фильма и пользователя
        verifyFilmAndUser(currentList, filmId, userId);

        //Если пользователь уже поставил лайк фильму выбрасываем исключение
        if (currentList.stream().filter(f -> f.getId() == filmId).findFirst().orElseThrow().getLikes().contains(userId)) {
    ...
    private void verifyFilmAndUser(List<Film> list, int filmId, int userId) {
        //Если фильма нет в базе выбрасываем исключение
        if(list.stream().noneMatch(f -> f.getId() == filmId)) {
        ...
        if(list.stream().noneMatch(u -> u.getId() == userId)) {
